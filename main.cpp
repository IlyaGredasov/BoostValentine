...
http::response<http::string_body> make_json_response(const http::request<http::string_body> &request) {
	return make_response(
	    http::status::ok,
	    "application/json",
	    boost::json::serialize(boost::json::object{{"confession", "This is the strangest way to tell her you love her"}}),
	    request);
}
...
int main() {
	try {
		...
		std::cout << "open localhost:8080" << std::endl;
		while (true) {
			...
			http::response<http::string_body> response =
			    (request.method() == http::verb::get && request.target() == "/")
			        ? make_json_response(request)
